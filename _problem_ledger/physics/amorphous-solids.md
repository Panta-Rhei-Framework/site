---
layout: "problem-ledger-item"
render_with_liquid: false
title: "Amorphous solids"
permalink: "/program/research-agenda/problem-ledger/physics/amorphous-solids/"
lane: "program"
v2_lane: "program"
section: "research-agenda"
type: "Problem Ledger Item"
status: "canonical"
summary_short: "Physics stress-test entry: Amorphous solids."
generated_from: "corpus/problem-ledger"
projection_version: "v1.0"
canonical_source: "corpus/problem-ledger"
do_not_edit: true
canonical_problem_id: "phys-amorphous-solids"
id: "phys-amorphous-solids"
short_title: "Amorphous solids"
slug: "amorphous-solids"
domain: "physics"
domain_slug: "physics"
display_domain: "Physics"
subdomain: "condensed-matter-physics"
problem_type: "external_open_problem"
visibility: "public"
source:
  source_class: "master_wholesale_import"
  source_name: "wikipedia_list_of_unsolved_problems_in_physics"
  source_page: "https://en.wikipedia.org/wiki/List_of_unsolved_problems_in_physics"
  source_permalink: "https://en.wikipedia.org/wiki/List_of_unsolved_problems_in_physics?oldid=1349830343"
  source_revision_id: "1349830343"
  source_retrieved_at: "2026-04-24T23:54:38Z"
  source_revision_timestamp: "2026-04-19T03:02:56Z"
  source_item_anchor: "amorphous-solids"
  source_line: 189
  source_section:
    - "Condensed matter physics"
  import_rule: "wholesale_pinned_revision"
  imported_unfiltered: true
  supplemental: false
  inclusion_rationale: "Part of a pinned Wikipedia master ledger import."
  license: "CC BY-SA 4.0 and GFDL"
  license_url: "https://en.wikipedia.org/wiki/Wikipedia:Copyrights"
  attribution: "Wikipedia contributors"
  source_id: "wikipedia-unsolved-physics-1349830343"
  source_url: "https://en.wikipedia.org/wiki/List_of_unsolved_problems_in_physics"
  source_entry_ids:
    - "wikipedia-physics-amorphous-solids"
  source_entry_records:
    -
      source_entry_id: "wikipedia-physics-amorphous-solids"
      source_name: "wikipedia_list_of_unsolved_problems_in_physics"
      source_display: "Wikipedia: List of unsolved problems in physics"
      source_url: "https://en.wikipedia.org/wiki/List_of_unsolved_problems_in_physics"
      source_permalink: "https://en.wikipedia.org/wiki/List_of_unsolved_problems_in_physics?oldid=1349830343"
      source_revision_id: "1349830343"
      source_revision_timestamp: "2026-04-19T03:02:56Z"
      source_title: "Amorphous solids"
      source_section:
        - "Condensed matter physics"
      source_line: 189
      source_text: "Amorphous solids: What is the nature of the glass transition between a fluid or regular solid and a glassy phase? What are the physical processes giving rise to the general properties of glasses and the glass transition?"
      problem_statement: "What is the nature of the glass transition between a fluid or regular solid and a glassy phase? What are the physical processes giving rise to the general properties of glasses and the glass transition?"
      source_questions: []
      source_links:
        -
          label: "Amorphous solid"
          url: "https://en.wikipedia.org/wiki/Amorphous_solid"
          target: "Amorphous solid"
        -
          label: "glass transition"
          url: "https://en.wikipedia.org/wiki/glass_transition"
          target: "glass transition"
        -
          label: "phase"
          url: "https://en.wikipedia.org/wiki/phase_(matter)"
          target: "phase (matter)"
        -
          label: "The New York Times"
          url: "https://en.wikipedia.org/wiki/The_New_York_Times"
          target: "The New York Times"
        -
          label: "Science"
          url: "https://en.wikipedia.org/wiki/Science_(journal)"
          target: "Science (journal)"
  source_links:
    -
      label: "Amorphous solid"
      url: "https://en.wikipedia.org/wiki/Amorphous_solid"
      target: "Amorphous solid"
    -
      label: "glass transition"
      url: "https://en.wikipedia.org/wiki/glass_transition"
      target: "glass transition"
    -
      label: "phase"
      url: "https://en.wikipedia.org/wiki/phase_(matter)"
      target: "phase (matter)"
    -
      label: "The New York Times"
      url: "https://en.wikipedia.org/wiki/The_New_York_Times"
      target: "The New York Times"
    -
      label: "Science"
      url: "https://en.wikipedia.org/wiki/Science_(journal)"
      target: "Science (journal)"
  content_audit:
    status: "source_enriched"
    audit_date: "2026-04-28"
    source_entry_count: 1
    missing_source_entries: []
    description_source: "pinned_source_entry"
program:
  tier: "unclassified"
  agenda_role: "stress_test"
  expressibility_status: "unknown"
  result_status: "not_yet_classified"
  recovery_status: ""
  priority: "unclassified"
  notes: "Mechanically imported from a pinned Wikipedia master ledger; pending editorial classification."
related:
  corpus_items: []
  results: []
  verify: []
  publications: []
  world_readout: []
  recovery_requirements: []
  construction_steps: []
mapping:
  mapping_status: "promoted"
  mapping_target: "phys-amorphous-solids"
  mapped_from_source_entries:
    - "wikipedia-physics-amorphous-solids"
  merged_source_entries: []
  excluded_source_entries: []
  mapping_notes: "Promoted in Problem Ledger v1.0 broad public projection."
cross_domain_relations: []
version:
  item_version: "1.0.0"
  introduced_in: "problem-ledger-wikipedia-import-v1"
  last_modified: "2026-04-28"
  change_summary: "Enriched with source-derived Problem Ledger content audit fields."
tags:
  - "physics"
  - "wikipedia-physics"
  - "amorphous-solids"
  - "condensed-matter-physics"
search_keywords:
  - "physics"
  - "wikipedia-physics"
  - "amorphous-solids"
  - "condensed-matter-physics"
  - "Amorphous solids"
url: "/program/research-agenda/problem-ledger/physics/amorphous-solids/"
results_mirror:
  exists: false
  result_id: ""
  result_status: "not_yet_classified"
  result_url: ""
verify_links:
  exists: false
  verification_mode: ""
  verify_url: ""
---

## Problem Statement

What is the nature of the glass transition between a fluid or regular solid and a glassy phase? What are the physical processes giving rise to the general properties of glasses and the glass transition?

## Source Links

- [Amorphous solid](https://en.wikipedia.org/wiki/Amorphous_solid)
- [glass transition](https://en.wikipedia.org/wiki/glass_transition)
- [phase](https://en.wikipedia.org/wiki/phase_(matter))
- [The New York Times](https://en.wikipedia.org/wiki/The_New_York_Times)
- [Science](https://en.wikipedia.org/wiki/Science_(journal))

## Source Trace

- `wikipedia-physics-amorphous-solids` — Wikipedia: List of unsolved problems in physics; section: Condensed matter physics; source line: 189; revision: 1349830343.

## Why This Problem Is in Scope

This item is included because it belongs to the declared Problem Ledger source scope for Physics. Import means visibility and accountable classification, not endorsement, solution, priority, or external acceptance.
