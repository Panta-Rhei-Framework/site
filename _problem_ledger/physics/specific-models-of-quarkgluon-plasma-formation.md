---
layout: "problem-ledger-item"
render_with_liquid: false
title: "Specific models of quark–gluon plasma formation"
permalink: "/program/research-agenda/problem-ledger/physics/specific-models-of-quarkgluon-plasma-formation/"
lane: "program"
v2_lane: "program"
section: "research-agenda"
type: "Problem Ledger Item"
status: "canonical"
summary_short: "Physics stress-test entry: Specific models of quark–gluon plasma formation."
generated_from: "corpus/problem-ledger"
projection_version: "v1.0"
canonical_source: "corpus/problem-ledger"
do_not_edit: true
canonical_problem_id: "phys-specific-models-of-quarkgluon-plasma-formation"
id: "phys-specific-models-of-quarkgluon-plasma-formation"
short_title: "Specific models of quark–gluon plasma formation"
slug: "specific-models-of-quarkgluon-plasma-formation"
domain: "physics"
domain_slug: "physics"
display_domain: "Physics"
subdomain: "nuclear-physics"
problem_type: "external_open_problem"
visibility: "public"
source:
  source_class: "master_wholesale_import"
  source_name: "wikipedia_list_of_unsolved_problems_in_physics"
  source_page: "https://en.wikipedia.org/wiki/List_of_unsolved_problems_in_physics"
  source_permalink: "https://en.wikipedia.org/wiki/List_of_unsolved_problems_in_physics?oldid=1349830343"
  source_revision_id: "1349830343"
  source_retrieved_at: "2026-04-24T23:54:38Z"
  source_revision_timestamp: "2026-04-19T03:02:56Z"
  source_item_anchor: "specific-models-of-quarkgluon-plasma-formation"
  source_line: 178
  source_section:
    - "Nuclear physics"
  import_rule: "wholesale_pinned_revision"
  imported_unfiltered: true
  supplemental: false
  inclusion_rationale: "Part of a pinned Wikipedia master ledger import."
  license: "CC BY-SA 4.0 and GFDL"
  license_url: "https://en.wikipedia.org/wiki/Wikipedia:Copyrights"
  attribution: "Wikipedia contributors"
  source_id: "wikipedia-unsolved-physics-1349830343"
  source_url: "https://en.wikipedia.org/wiki/List_of_unsolved_problems_in_physics"
  source_entry_ids:
    - "wikipedia-physics-specific-models-of-quarkgluon-plasma-formation"
  source_entry_records:
    -
      source_entry_id: "wikipedia-physics-specific-models-of-quarkgluon-plasma-formation"
      source_name: "wikipedia_list_of_unsolved_problems_in_physics"
      source_display: "Wikipedia: List of unsolved problems in physics"
      source_url: "https://en.wikipedia.org/wiki/List_of_unsolved_problems_in_physics"
      source_permalink: "https://en.wikipedia.org/wiki/List_of_unsolved_problems_in_physics?oldid=1349830343"
      source_revision_id: "1349830343"
      source_revision_timestamp: "2026-04-19T03:02:56Z"
      source_title: "Specific models of quark–gluon plasma formation"
      source_section:
        - "Nuclear physics"
      source_line: 178
      source_text: "Specific models of quark–gluon plasma formation: Do gluons saturate when their occupation number is large? Do gluons form a dense system called colour glass condensate? What are the signatures and evidences for the Balitsky–Fadin–Kuarev–Lipatov (BFKL), Balitsky–Kovchegov (BK), Catani–Ciafaloni–Fiorani–Marchesini (CCFM) evolution equations?"
      problem_statement: "Do gluons saturate when their occupation number is large? Do gluons form a dense system called colour glass condensate? What are the signatures and evidences for the Balitsky–Fadin–Kuarev–Lipatov (BFKL), Balitsky–Kovchegov (BK), Catani–Ciafaloni–Fiorani–Marchesini (CCFM) evolution equations?"
      source_questions: []
      source_links:
        -
          label: "gluon"
          url: "https://en.wikipedia.org/wiki/gluon"
          target: "gluon"
        -
          label: "colour glass condensate"
          url: "https://en.wikipedia.org/wiki/Color_Glass_Condensate"
          target: "Color Glass Condensate"
  source_links:
    -
      label: "gluon"
      url: "https://en.wikipedia.org/wiki/gluon"
      target: "gluon"
    -
      label: "colour glass condensate"
      url: "https://en.wikipedia.org/wiki/Color_Glass_Condensate"
      target: "Color Glass Condensate"
  content_audit:
    status: "source_enriched"
    audit_date: "2026-04-28"
    source_entry_count: 1
    missing_source_entries: []
    description_source: "pinned_source_entry"
program:
  tier: "unclassified"
  agenda_role: "stress_test"
  expressibility_status: "unknown"
  result_status: "not_yet_classified"
  recovery_status: ""
  priority: "unclassified"
  notes: "Mechanically imported from a pinned Wikipedia master ledger; pending editorial classification."
related:
  corpus_items: []
  results: []
  verify: []
  publications: []
  world_readout: []
  recovery_requirements: []
  construction_steps: []
mapping:
  mapping_status: "promoted"
  mapping_target: "phys-specific-models-of-quarkgluon-plasma-formation"
  mapped_from_source_entries:
    - "wikipedia-physics-specific-models-of-quarkgluon-plasma-formation"
  merged_source_entries: []
  excluded_source_entries: []
  mapping_notes: "Promoted in Problem Ledger v1.0 broad public projection."
cross_domain_relations: []
version:
  item_version: "1.0.0"
  introduced_in: "problem-ledger-wikipedia-import-v1"
  last_modified: "2026-04-28"
  change_summary: "Enriched with source-derived Problem Ledger content audit fields."
tags:
  - "physics"
  - "wikipedia-physics"
  - "specific-models-of-quarkgluon-plasma-formation"
  - "nuclear-physics"
search_keywords:
  - "physics"
  - "wikipedia-physics"
  - "specific-models-of-quarkgluon-plasma-formation"
  - "nuclear-physics"
  - "Specific models of quark–gluon plasma formation"
url: "/program/research-agenda/problem-ledger/physics/specific-models-of-quarkgluon-plasma-formation/"
results_mirror:
  exists: false
  result_id: ""
  result_status: "not_yet_classified"
  result_url: ""
verify_links:
  exists: false
  verification_mode: ""
  verify_url: ""
---

## Problem Statement

Do gluons saturate when their occupation number is large? Do gluons form a dense system called colour glass condensate? What are the signatures and evidences for the Balitsky–Fadin–Kuarev–Lipatov (BFKL), Balitsky–Kovchegov (BK), Catani–Ciafaloni–Fiorani–Marchesini (CCFM) evolution equations?

## Source Links

- [gluon](https://en.wikipedia.org/wiki/gluon)
- [colour glass condensate](https://en.wikipedia.org/wiki/Color_Glass_Condensate)

## Source Trace

- `wikipedia-physics-specific-models-of-quarkgluon-plasma-formation` — Wikipedia: List of unsolved problems in physics; section: Nuclear physics; source line: 178; revision: 1349830343.

## Why This Problem Is in Scope

This item is included because it belongs to the declared Problem Ledger source scope for Physics. Import means visibility and accountable classification, not endorsement, solution, priority, or external acceptance.
