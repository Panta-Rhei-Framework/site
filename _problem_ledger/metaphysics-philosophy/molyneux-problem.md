---
layout: "problem-ledger-item"
render_with_liquid: false
title: "Molyneux problem"
permalink: "/program/research-agenda/problem-ledger/metaphysics-philosophy/molyneux-problem/"
lane: "program"
v2_lane: "program"
section: "research-agenda"
type: "Problem Ledger Item"
status: "canonical"
summary_short: "Metaphysics / Philosophy stress-test entry: Molyneux problem."
generated_from: "corpus/problem-ledger"
projection_version: "v1.0"
canonical_source: "corpus/problem-ledger"
do_not_edit: true
canonical_problem_id: "meta-molyneux-problem"
id: "meta-molyneux-problem"
short_title: "Molyneux problem"
slug: "molyneux-problem"
domain: "metaphysics_philosophy"
domain_slug: "metaphysics-philosophy"
display_domain: "Metaphysics / Philosophy"
subdomain: "epistemology"
problem_type: "external_open_problem"
visibility: "public"
source:
  source_class: "master_wholesale_import"
  source_name: "wikipedia_list_of_philosophical_problems"
  source_page: "https://en.wikipedia.org/wiki/List_of_philosophical_problems"
  source_permalink: "https://en.wikipedia.org/wiki/List_of_philosophical_problems?oldid=1338262080"
  source_revision_id: "1338262080"
  source_retrieved_at: "2026-04-24T23:54:39Z"
  source_revision_timestamp: "2026-02-14T02:33:54Z"
  source_item_anchor: "molyneux-problem"
  source_line: 65
  source_section:
    - "Epistemology"
  import_rule: "wholesale_pinned_revision"
  imported_unfiltered: true
  supplemental: false
  inclusion_rationale: "Part of a pinned Wikipedia master ledger import."
  license: "CC BY-SA 4.0 and GFDL"
  license_url: "https://en.wikipedia.org/wiki/Wikipedia:Copyrights"
  attribution: "Wikipedia contributors"
  source_id: "wikipedia-philosophical-problems-1338262080"
  source_url: "https://en.wikipedia.org/wiki/List_of_philosophical_problems"
  source_entry_ids:
    - "wikipedia-philosophy-molyneux-problem"
  source_entry_records:
    -
      source_entry_id: "wikipedia-philosophy-molyneux-problem"
      source_name: "wikipedia_list_of_philosophical_problems"
      source_display: "Wikipedia: List of philosophical problems"
      source_url: "https://en.wikipedia.org/wiki/List_of_philosophical_problems"
      source_permalink: "https://en.wikipedia.org/wiki/List_of_philosophical_problems?oldid=1338262080"
      source_revision_id: "1338262080"
      source_revision_timestamp: "2026-02-14T02:33:54Z"
      source_title: "Molyneux problem"
      source_section:
        - "Epistemology"
      source_line: 65
      source_text: "Molyneux problem"
      problem_statement: "The Molyneux problem dates back to the following question posed by William Molyneux to John Locke in the 17th century: if a man born blind, and able to distinguish by touch between a cube and a globe, were made to see, could he now tell by sight which was the cube and which the globe, before he touched them? The problem raises fundamental issues in epistemology and the philosophy of mind, and was widely discussed after Locke included it in the second edition of his Essay Concerning Human Understanding.Locke, John. An Essay Concerning Human Understanding, Book 2, Chapter 9 A similar problem was also addressed earlier in the 12th century by Ibn Tufail (Abubacer), in his philosophical novel, Hayy ibn Yaqdhan (Philosophus Autodidactus). His version of the problem, however, dealt mainly with colors rather than shapes.Muhammad ibn Abd al-Malik Ibn Tufayl and Léon Gauthier (1981), Risalat Hayy ibn Yaqzan, p. 5, Editions de la Méditerranée Empirical studies of human subjects who gained vision after extended congenital blindness have provided clear evidence. The landmark 2011 study by Held et al. demonstrated that subjects were unable to immediately link objects known by touch to their visual appearance. Instead, they gradually developed this ability over days or months. Thus, a person newly given sight cannot immediately identify shapes they previously knew only by touch. The ability to associate tactile and visual information requires learning and experience. Another study suggests, however, that the results might have been due to a visual deficit, rather than a true lack of intermodal neural connections"
      source_questions: []
      source_links:
        -
          label: "William Molyneux"
          url: "https://en.wikipedia.org/wiki/William_Molyneux"
          target: "William Molyneux"
        -
          label: "John Locke"
          url: "https://en.wikipedia.org/wiki/John_Locke"
          target: "John Locke"
        -
          label: "cube"
          url: "https://en.wikipedia.org/wiki/cube"
          target: "cube"
        -
          label: "globe"
          url: "https://en.wikipedia.org/wiki/globe"
          target: "globe"
        -
          label: "epistemology"
          url: "https://en.wikipedia.org/wiki/epistemology"
          target: "epistemology"
        -
          label: "philosophy of mind"
          url: "https://en.wikipedia.org/wiki/philosophy_of_mind"
          target: "philosophy of mind"
        -
          label: "Essay Concerning Human Understanding"
          url: "https://en.wikipedia.org/wiki/Essay_Concerning_Human_Understanding"
          target: "Essay Concerning Human Understanding"
        -
          label: "An Essay Concerning Human Understanding"
          url: "https://en.wikipedia.org/wiki/An_Essay_Concerning_Human_Understanding"
          target: "An Essay Concerning Human Understanding"
        -
          label: "Ibn Tufail"
          url: "https://en.wikipedia.org/wiki/Ibn_Tufail"
          target: "Ibn Tufail"
        -
          label: "philosophical novel"
          url: "https://en.wikipedia.org/wiki/philosophical_novel"
          target: "philosophical novel"
        -
          label: "Hayy ibn Yaqdhan"
          url: "https://en.wikipedia.org/wiki/Hayy_ibn_Yaqdhan"
          target: "Hayy ibn Yaqdhan"
        -
          label: "Ibn Tufayl"
          url: "https://en.wikipedia.org/wiki/Ibn_Tufayl"
          target: "Ibn Tufayl"
  source_links:
    -
      label: "William Molyneux"
      url: "https://en.wikipedia.org/wiki/William_Molyneux"
      target: "William Molyneux"
    -
      label: "John Locke"
      url: "https://en.wikipedia.org/wiki/John_Locke"
      target: "John Locke"
    -
      label: "cube"
      url: "https://en.wikipedia.org/wiki/cube"
      target: "cube"
    -
      label: "globe"
      url: "https://en.wikipedia.org/wiki/globe"
      target: "globe"
    -
      label: "epistemology"
      url: "https://en.wikipedia.org/wiki/epistemology"
      target: "epistemology"
    -
      label: "philosophy of mind"
      url: "https://en.wikipedia.org/wiki/philosophy_of_mind"
      target: "philosophy of mind"
    -
      label: "Essay Concerning Human Understanding"
      url: "https://en.wikipedia.org/wiki/Essay_Concerning_Human_Understanding"
      target: "Essay Concerning Human Understanding"
    -
      label: "An Essay Concerning Human Understanding"
      url: "https://en.wikipedia.org/wiki/An_Essay_Concerning_Human_Understanding"
      target: "An Essay Concerning Human Understanding"
    -
      label: "Ibn Tufail"
      url: "https://en.wikipedia.org/wiki/Ibn_Tufail"
      target: "Ibn Tufail"
    -
      label: "philosophical novel"
      url: "https://en.wikipedia.org/wiki/philosophical_novel"
      target: "philosophical novel"
    -
      label: "Hayy ibn Yaqdhan"
      url: "https://en.wikipedia.org/wiki/Hayy_ibn_Yaqdhan"
      target: "Hayy ibn Yaqdhan"
    -
      label: "Ibn Tufayl"
      url: "https://en.wikipedia.org/wiki/Ibn_Tufayl"
      target: "Ibn Tufayl"
  content_audit:
    status: "source_enriched"
    audit_date: "2026-04-28"
    source_entry_count: 1
    missing_source_entries: []
    description_source: "pinned_source_entry"
program:
  tier: "unclassified"
  agenda_role: "stress_test"
  expressibility_status: "unknown"
  result_status: "not_yet_classified"
  recovery_status: ""
  priority: "unclassified"
  notes: "Mechanically imported from a pinned Wikipedia master ledger; pending editorial classification."
related:
  corpus_items: []
  results: []
  verify: []
  publications: []
  world_readout: []
  recovery_requirements: []
  construction_steps: []
mapping:
  mapping_status: "promoted"
  mapping_target: "meta-molyneux-problem"
  mapped_from_source_entries:
    - "wikipedia-philosophy-molyneux-problem"
  merged_source_entries: []
  excluded_source_entries: []
  mapping_notes: "Promoted in Problem Ledger v1.0 broad public projection."
cross_domain_relations: []
version:
  item_version: "1.0.0"
  introduced_in: "problem-ledger-wikipedia-import-v1"
  last_modified: "2026-04-28"
  change_summary: "Enriched with source-derived Problem Ledger content audit fields."
tags:
  - "metaphysics_philosophy"
  - "wikipedia-philosophy"
  - "molyneux-problem"
  - "epistemology"
search_keywords:
  - "metaphysics_philosophy"
  - "wikipedia-philosophy"
  - "molyneux-problem"
  - "epistemology"
  - "Molyneux problem"
url: "/program/research-agenda/problem-ledger/metaphysics-philosophy/molyneux-problem/"
results_mirror:
  exists: false
  result_id: ""
  result_status: "not_yet_classified"
  result_url: ""
verify_links:
  exists: false
  verification_mode: ""
  verify_url: ""
---

## Problem Statement

The Molyneux problem dates back to the following question posed by William Molyneux to John Locke in the 17th century: if a man born blind, and able to distinguish by touch between a cube and a globe, were made to see, could he now tell by sight which was the cube and which the globe, before he touched them? The problem raises fundamental issues in epistemology and the philosophy of mind, and was widely discussed after Locke included it in the second edition of his Essay Concerning Human Understanding.Locke, John. An Essay Concerning Human Understanding, Book 2, Chapter 9 A similar problem was also addressed earlier in the 12th century by Ibn Tufail (Abubacer), in his philosophical novel, Hayy ibn Yaqdhan (Philosophus Autodidactus). His version of the problem, however, dealt mainly with colors rather than shapes.Muhammad ibn Abd al-Malik Ibn Tufayl and Léon Gauthier (1981), Risalat Hayy ibn Yaqzan, p. 5, Editions de la Méditerranée Empirical studies of human subjects who gained vision after extended congenital blindness have provided clear evidence. The landmark 2011 study by Held et al. demonstrated that subjects were unable to immediately link objects known by touch to their visual appearance. Instead, they gradually developed this ability over days or months. Thus, a person newly given sight cannot immediately identify shapes they previously knew only by touch. The ability to associate tactile and visual information requires learning and experience. Another study suggests, however, that the results might have been due to a visual deficit, rather than a true lack of intermodal neural connections

## Source Links

- [William Molyneux](https://en.wikipedia.org/wiki/William_Molyneux)
- [John Locke](https://en.wikipedia.org/wiki/John_Locke)
- [cube](https://en.wikipedia.org/wiki/cube)
- [globe](https://en.wikipedia.org/wiki/globe)
- [epistemology](https://en.wikipedia.org/wiki/epistemology)
- [philosophy of mind](https://en.wikipedia.org/wiki/philosophy_of_mind)
- [Essay Concerning Human Understanding](https://en.wikipedia.org/wiki/Essay_Concerning_Human_Understanding)
- [An Essay Concerning Human Understanding](https://en.wikipedia.org/wiki/An_Essay_Concerning_Human_Understanding)
- [Ibn Tufail](https://en.wikipedia.org/wiki/Ibn_Tufail)
- [philosophical novel](https://en.wikipedia.org/wiki/philosophical_novel)
- [Hayy ibn Yaqdhan](https://en.wikipedia.org/wiki/Hayy_ibn_Yaqdhan)
- [Ibn Tufayl](https://en.wikipedia.org/wiki/Ibn_Tufayl)

## Source Trace

- `wikipedia-philosophy-molyneux-problem` — Wikipedia: List of philosophical problems; section: Epistemology; source line: 65; revision: 1338262080.

## Why This Problem Is in Scope

This item is included because it belongs to the declared Problem Ledger source scope for Metaphysics / Philosophy. Import means visibility and accountable classification, not endorsement, solution, priority, or external acceptance.
