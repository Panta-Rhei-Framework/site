---
layout: "program-doc"
title: "Why are all leaves the size they are, and not an order of magnitude larger or smaller?"
permalink: "/results/problem-ledger-answers/life/why-are-all-leaves-the-size-they-are-and-not-an-order-of-magnitude-larger-or-smaller/"
lane: "results"
v2_lane: "results"
type: "Problem Ledger Answer"
status: "Canonical"
summary_short: "Current Results-side stance for Why are all leaves the size they are, and not an order of magnitude larger or smaller?."
canonical_problem_id: "life-biology-why-are-all-leaves-the-size-they-are-and-not-an-order-of-magnitude-larger-or-smaller"
---

## Status Separation

<div class="notice note"><strong>Status note.</strong> This page reports the current program stance. It does not imply external acceptance unless explicitly stated.</div>

- Internal stance: **Not yet touched**
- Verification route: **Pending**
- External status: **Not externally reviewed**

## Problem

Why are all leaves the size they are, and not an order of magnitude larger or smaller?

## Source

- Source: https://en.wikipedia.org/wiki/List_of_unsolved_problems_in_biology
- Import rule: `wholesale_pinned_revision`
- Program ledger item: [life-biology-why-are-all-leaves-the-size-they-are-and-not-an-order-of-magnitude-larger-or-smaller](/program/research-agenda/problem-ledger/life/why-are-all-leaves-the-size-they-are-and-not-an-order-of-magnitude-larger-or-smaller/)

## Current Program Stance

- Result status: **Not yet touched**
- Tier: `unclassified`
- Agenda role: `stress_test`
- Expressibility: `unknown`

## Related Construction Steps

- Construction Spine mapping pending.

## Related Results

- Dedicated Result page pending.

## Verify This Answer

Dedicated Verify surface pending.

## What remains open

Residual boundaries remain public unless a linked Result page explicitly closes them with status-marked evidence.
